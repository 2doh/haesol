...
export const findTeacherId = async (data) => {
    try {
        const response = await axios.get(`/api/teacher/find_id?name=${data.name}&phone=${data.phone}`)
        return response
    } catch (error) {
        return "error"
    }
...
export const findTeacherPass = async (data) => {
    try {
        const res = await axios.get(`/api/teacher/find_pwd?id=${data.id}&phone=${data.phone}`)
        return res
    } catch (error) {
        return "err"
    }
