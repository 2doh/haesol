...
const FindId = () => {
  ...
  return (
    ...
    <form className="login-wrap-panel">
      ...
      <button className="login-wrap-panel-loginbt" onClick={e=>handleOnSubmit(e)}>아이디찾기</button>
      </> : <div className="login-wrap-panel-getuserid">
      <div className="login-panel-getinfo">아이디는 {getUserId} 입니다</div>
      <button className="login-wrap-panel-loginbt" onClick={()=>{navi("/login")}}>확인</button>
        </div>}
    </form>
    </div>
    </div>
